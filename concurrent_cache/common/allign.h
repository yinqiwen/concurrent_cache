...
template <typename T>
T align_to_next_power_of_two(T x) {
  x--;
  x |= x >> 1;
  x |= x >> 2;
  x |= x >> 4;
...
T is_prime(T n) {
  ...
  if (n == 2) return 1;                                                   // 2是唯一偶数质数
  if (n % 2 == 0) return 0;                                               // 排除其他偶数
  T max_divisor = static_cast<T>(std::sqrt(static_cast<double>(n))) + 1;  // 计算平方根向上取整 [[6]]
  for (int i = 3; i < max_divisor; i += 2) {
    if (n % i == 0) return 0;  // 存在因数则非质数
  }
  return 1;
}
...
template <typename T>
T next_prime(T n) {
  if (n <= 1) return 2;       // 小于等于1时返回最小质数2
  if (is_prime(n)) return n;  // 如果n本身是质数，直接返回
  // 从下一个奇数开始检查 [[9]]
  T candidate = (n % 2 == 0) ? n + 1 : n + 2;
  while (1) {
    if (is_prime(candidate)) return candidate;
    candidate += 2;  // 仅检查奇数候选 [[6]]
  }
}
